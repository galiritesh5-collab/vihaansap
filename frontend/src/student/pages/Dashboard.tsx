...
/**
 * Student Dashboard – robust version with explicit loading, error, and empty states.
 * Uses existing styling utilities; no new design system introduced.
 */
export default function Dashboard() {
  ...
  // Basic loading guard – DB and profile are expected to be available quickly.
  ...
  // Derived data – defensively handle missing collections.
  const myBatches = enrolledBatches ?? [];
  const mySessions = (db.batchSessions ?? []).filter(
    (s) => myBatches.some((b) => b.id === s.batchId) && isTargetedToStudent(s, studentProfile)
  );
  ...
  // Pending feedback (sessions already completed without feedback)
  const pendingFeedbackSessions = pastSessions.filter(
    (s) => !db.sessionFeedback?.some((f) => f.sessionId === s.id && f.studentId === studentProfile?.id)
  );

  // Review requests
  const pendingReviewRequests = (db.notifications || [])
    .map((notification: any) => {
      const campaign = (db.reviewCampaigns || []).find((c: any) => c.id === notification.targetId);
      return {
        notification,
        campaign,
    ...
    .sort((a: any, b: any) => {
      const dateA = new Date(b.campaign?.createdAt || b.notification.createdAt || b.notification.date || 0).getTime();
      const dateB = new Date(a.campaign?.createdAt || a.notification.createdAt || a.notification.date || 0).getTime();
      return dateA - dateB;
    });

  // Summary cards data – handle empty gracefully.
  const stats = [
  ...
  // Registration pending – unchanged from original.
    ...
    return (
      <div className="p-4 sm:p-8 flex items-center justify-center min-h-[70vh]">
        <div className="bg-white rounded-2xl p-8 max-w-lg w-full text-center border border-slate-100 shadow-xl relative overflow-hidden">
          ...
          <h2 className="text-2xl font-display font-extrabold text-slate-800 tracking-tight relative z-10 mb-3">Registration Successful</h2>
          <p className="text-slate-600 text-sm leading-relaxed relative z-10 mb-2">Your account has been created successfully.</p>
          <p className="text-slate-500 text-sm leading-relaxed relative z-10 mb-8">
            Our team will verify your enrollment and assign your purchased course or batch. Once assigned, your learning dashboard will automatically become available.
          </p>
          <a href="https://wa.me/917075999336" target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-[#1763B6] hover:bg-[#145096] text-white font-bold rounded-xl transition-colors shadow-sm relative z-10">
      ...
      <div className="p-4 sm:p-8 space-y-6 sm:space-y-8 max-w-7xl mx-auto">
                  ...
                  <div className="flex items-start sm:items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-red-800 mt-1 max-w-2xl">
                        {n.message || 'Your mentor has requested you to share your learning experience. Your feedback helps improve our training programs.'}
                      </p>
                    </div>
                  </div>
                  <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            {/* Continue Learning (My Active Batches) */}
                    ...
                    return (
                      <div key={batch.id} className="flex flex-col sm:flex-row gap-6 items-center sm:items-start mb-6 last:mb-0">
                        ...
                        <div className="flex-1 space-y-3 w-full">
                          <div>
                            <span className="text-xs font-bold uppercase tracking-wider text-[#1763B6] bg-blue-50 px-2 py-1 rounded-md">{batch.status}</span>
                          ...
                          <Link
                            to="/student/courses"
                            className="inline-block w-full sm:w-auto bg-[#1763B6] hover:bg-[#145096] text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors mt-2 text-center"
                          >
                            Go to Workspace
                          </Link>
                        </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              <div className="p-4 sm:p-6">
                {upcomingSessions.length > 0 ? (
                  upcomingSessions.slice(0, 2).map((cls) => {
                    ...
                    return (
                      <div
                        key={cls.id}
                        className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-xl border border-orange-100 bg-orange-50/50 mb-4 last:mb-0"
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
                              <span className="font-medium text-slate-700">{cls.date || new Date(cls.sessionDateTime).toLocaleDateString()}</span> at{' '}
                              <span className="font-medium text-slate-700">{cls.time || new Date(cls.sessionDateTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>{' '}• {batch?.course} • Mentor: {batch?.mentor || 'Assigned mentor'}
                            </p>
                            <p className="mt-1 text-xs font-semibold text-orange-700">
                              {cls.status || 'Upcoming'}{recording || cls.recordingUrl ? ' • Recording available' : ''}
                            </p>
                          </div>
                        </div>
                        {cls.meetingLink ? (
                        ...
                        {(recording?.videoUrl || cls.recordingUrl) && (
                          <a href={recording?.videoUrl || cls.recordingUrl} target="_blank" rel="noreferrer" className="w-full sm:w-auto px-4 py-2 border border-orange-300 text-orange-700 text-sm font-bold rounded-lg text-center">
                            View Recording
                          </a>
                        )}
                      </div>
            ...
            <div className="bg-gradient-to-br from-[#1763B6] to-[#0C3E7B] rounded-2xl p-6 text-white shadow-md relative overflow-hidden">
              ...
              <a href="https://wa.me/917075999336" target="_blank" rel="noreferrer" className="block text-center w-full bg-white text-[#1763B6] font-bold py-2.5 rounded-lg hover:bg-slate-50 transition-colors text-sm">
                Contact Support
              </a>
            </div>
          ...
          <div className="space-y-6">
            {/* Placeholder for additional components (e.g., Materials, Doubt Support) – can be expanded later */}
          </div>
        </div>
